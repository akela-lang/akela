...
void test_parse_assign2()
{
	...
	expect_int_equal(right2->type, Cob_ast_type_number, "number2");
...
void test_parse_stmts()
{
	...
	Cob_ast* left = Ast_node_get(cu.root, 2);
	assert_ptr(left, "left");
...
void test_parse_stmts2()
{
	...
	assert_ptr(c, "ptr c");
	expect_int_equal(c->type, Cob_ast_type_number, "number c");
	expect_str(&c->value, "1", "1 c");

	Cob_ast* d = Ast_node_get(cu.root, 2);
	assert_ptr(d, "ptr d");
	expect_int_equal(d->type, Ast_type_minus, "minus d");

	Cob_ast* e = Ast_node_get(d, 0);
	assert_ptr(e, "ptr e");
	expect_int_equal(e->type, Cob_ast_type_number, "number e");
...
void test_parse_stmts3()
{
	...
	Cob_ast* d = Ast_node_get(cu.root, 4);
	assert_ptr(d, "ptr d");
	expect_int_equal(d->type, Ast_type_minus, "minus d");
	...
	Cob_ast* g = Ast_node_get(cu.root, 5);
	assert_ptr(g, "ptr g");
...
void test_parse_if()
{
	...
	Cob_ast* if_node = Ast_node_get(cu.root, 0);
	...
	Cob_ast* cb = Ast_node_get(if_node, 0);
	assert_ptr(cb, "ptr cb");
...
void test_parse_elseif()
{
	...
	Cob_ast* if_stmt = Ast_node_get(cu.root, 2);
	assert_ptr(if_stmt, "ptr if_stmt");
	...
	Cob_ast* plus = Ast_node_get(stmts, 0);
	assert_ptr(plus, "ptr plus");
	expect_int_equal(plus->type, Ast_type_plus, "plus");
	...
	Cob_ast* mult = Ast_node_get(stmts, 1);
	assert_ptr(mult, "ptr mult");
	expect_int_equal(mult->type, Ast_type_mult, "mult");
	...
	Cob_ast* stmts2 = Ast_node_get(cb2, 1);
	assert_ptr(stmts2, "ptr stmts2");
	expect_int_equal(stmts2->type, Ast_type_stmts, "parse_stmts stmts2");

	Cob_ast* num2 = Ast_node_get(stmts2, 0);
	assert_ptr(num2, "ptr num2");
	expect_int_equal(num2->type, Cob_ast_type_number, "number num2");
	expect_str(&num2->value, "1", "1 num2");

	Cob_ast* num3 = Ast_node_get(stmts2, 1);
	assert_ptr(num3, "ptr num3");
	expect_int_equal(num3->type, Cob_ast_type_number, "number num3");
...
void test_parse_else2()
{
	...
	Cob_ast* cond0 = Ast_node_get(cb0, 0);
	...
	Cob_ast* num0 = Ast_node_get(stmts0, 0);
	assert_ptr(num0, "ptr num0");
	expect_int_equal(num0->type, Cob_ast_type_number, "number num0");
	...
	Cob_ast* cond1 = Ast_node_get(cb1, 0);
	...
	Cob_ast* stmts1 = Ast_node_get(cb1, 1);
	assert_ptr(stmts1, "ptr stmts1");
	expect_int_equal(stmts1->type, Ast_type_stmts, "parse_stmts stmts1");

	Cob_ast* num1 = Ast_node_get(stmts1, 0);
	expect_int_equal(num1->type, Cob_ast_type_number, "number num1");
	expect_str(&num1->value, "20", "20 num1");

	/* else */
	Cob_ast* db = Ast_node_get(if_stmt, 2);
	assert_ptr(db, "ptr db");
	expect_int_equal(db->type, Ast_type_default_branch, "default branch db");

	Cob_ast* stmts2 = Ast_node_get(db, 0);
	assert_ptr(stmts2, "ptr stmts2");
	expect_int_equal(stmts2->type, Ast_type_stmts, "parse_stmts stmts2");

	Cob_ast* x = Ast_node_get(stmts2, 0);
	assert_ptr(x, "ptr x");
	expect_int_equal(x->type, Ast_type_id, "id x");
	expect_str(&x->value, "x", "x");

	Cob_ast* y = Ast_node_get(stmts2, 1);
	assert_ptr(y, "ptr y");
	expect_int_equal(y->type, Ast_type_id, "id y");
	expect_str(&y->value, "y", "y");
...
void test_parse_while()
{
	...
	Cob_ast* cond = Ast_node_get(node, 0);
	assert_ptr(cond, "ptr cond");
	expect_int_equal(cond->type, Ast_type_boolean, "boolean cond");
	expect_str(&cond->value, "true", "true cond");

	Cob_ast* stmts = Ast_node_get(node, 1);
	assert_ptr(stmts, "ptr parse_stmts");
	expect_int_equal(stmts->type, Ast_type_stmts, "parse_stmts stmts");

	Cob_ast* num = Ast_node_get(stmts, 0);
	assert_ptr(num, "ptr num");
	expect_int_equal(num->type, Cob_ast_type_number, "number num");
...
void test_parse_for_range()
{
	...
	/* i */
	Cob_ast* dec = Ast_node_get(node, 0);
	...
	/* start */
	Cob_ast* start = Ast_node_get(node, 1);
	assert_ptr(start, "ptr start");
	expect_int_equal(start->type, Cob_ast_type_number, "number start");
	expect_str(&start->value, "0", "0 start");

	/* end */
	Cob_ast* end = Ast_node_get(node, 2);
	assert_ptr(end, "ptr end");
	expect_int_equal(end->type, Cob_ast_type_number, "number end");
	expect_str(&end->value, "10", "10 end");

	/* stmts */
	Cob_ast* stmts = Ast_node_get(node, 3);
	assert_ptr(stmts, "ptr parse_stmts");
	expect_int_equal(stmts->type, Ast_type_stmts, "parse_stmts stmts");

	Cob_ast* stmt0 = Ast_node_get(stmts, 0);
	assert_ptr(stmt0, "ptr stmt0");
	expect_int_equal(stmt0->type, Cob_ast_type_number, "number stmt0");
...
void test_parse_for_iteration()
{
	...
	/* for */
	Cob_ast* node = Ast_node_get(cu.root, 1);
	assert_ptr(node, "ptr node");
	...
	Cob_ast* i_type_node = Ast_node_get(dec, 1);
...
void test_parse_let()
{
    ...
    Cob_ast* type_node = Ast_node_get(let, 1);
...
void test_parse_let2()
{
    ...
    Cob_ast* value = Ast_node_get(let_rseq, 0);
    assert_ptr(value, "ptr value");
    expect_int_equal(value->type, Cob_ast_type_number, "number");
...
void test_parse_let3()
{
    ...
    Cob_ast* id0 = Ast_node_get(let_lseq, 0);
    ...
    Cob_ast* id2 = Ast_node_get(let_lseq, 1);
    assert_ptr(id2, "ptr id2");
    ...
    Cob_ast* value0 = Ast_node_get(let_rseq, 0);
    assert_ptr(value0, "ptr value0");
    expect_int_equal(value0->type, Cob_ast_type_number, "type value0");
    expect_str(&value0->value, "1", "value value0");

    Cob_ast* value1 = Ast_node_get(let_rseq, 1);
    assert_ptr(value1, "ptr value1");
    expect_int_equal(value1->type, Cob_ast_type_number, "type value1");
...
void test_parse_stmts_mut() {
    ...
    Cob_ast *let_lhs = Ast_node_get(let_lseq, 0);
    ...
    Cob_ast *x = Ast_node_get(assign, 0);
    assert_ptr(x, "ptr x");
