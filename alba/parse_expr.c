...
enum result array_subscript(struct allocator* al, struct token_state* ts, struct dag_node** root)
{
	...
	if (t0 && t0->type == token_left_square_bracket) {
		...
		r = match(al, ts, token_left_square_bracket, "expecting array subscript operator");
		if (r == result_error) {
			goto function_error;
		}
		...
		r = match(al, ts, token_right_square_bracket, "expecting array subscript operator");
		if (r == result_error) {
			goto function_error;
		}
...
enum result array_subscript_prime(struct allocator* al, struct token_state* ts, struct dag_node* parent)
{
	...
	r = match(al, ts, token_right_square_bracket, "expected right square_bracket");
	if (r == result_error) {
		return r;
	}
