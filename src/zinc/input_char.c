...
bool Zinc_input_char_next(
    ...
    Zinc_location* loc)
{
    Zinc_input_char_next_interface *get_char = input_obj + input_vtable->next_offset;
    return (*get_char)(input_obj, c, loc);
}

void Zinc_input_char_repeat(void* input_obj, Zinc_input_char_vtable* input_vtable)
{
    Zinc_input_char_repeat_interface *repeat_char = input_obj + input_vtable->repeat_offset;
    (*repeat_char)(input_obj);
}

void Zinc_input_char_seek(void* input_obj, Zinc_input_char_vtable* input_vtable, Zinc_location* loc)
{
    Zinc_input_char_seek_interface *seek = input_obj + input_vtable->seek_offset;
    (*seek)(input_obj, loc);
}
