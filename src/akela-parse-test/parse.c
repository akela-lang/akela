...
void Apt_parse_suite(Apt_data* data, Zinc_string* path, Zinc_string* name);
void Apt_parse_test(
    Apt_data* data,
    Zinc_string* path,
    Apt_suite* suite,
    Lava_dom* dom,
    Zinc_string* name);
void Apt_parse_suite_meta(Apt_data* data, Apt_suite *suite, Cent_value* value);
void Apt_parse_test_meta(Apt_data* data, Apt_test* tc, Cent_value* value);
    ...
    while (node) {
        if (!Zinc_string_compare_str(&node->value, ".") && !Zinc_string_compare_str(&node->value, "..")) {
            ...
            if (Zinc_is_reg_file(&path)) {
                Apt_parse_suite(data, &path, &node->value);
            }
...
void Apt_parse_suite(Apt_data* data, Zinc_string* path, Zinc_string* name)
{
    ...
    Apt_suite* suite = NULL;
    Apt_suite_create(&suite);
    ...
    if (pr.root->kind == LAVA_DOM_HEADER) {
        ...
        while (item) {
            if (item->kind == LAVA_DOM_TEXT) {
                ...
                Apt_parse_suite_meta(data, suite, ct->primary->value);
...
void Apt__parse_test_suite_meta_prop(Zinc_string* name, Cent_value* prop)
{
    ...
    Apt_suite* suite = Apt__parse_test_suite_meta_prop_suite;
    if (Zinc_string_compare_str(name, "solo")) {
        if (prop->type == Cent_value_type_boolean) {
            suite->test->solo = prop->data.boolean;
        } else {
            Cent_ast* n = prop->n;
        ...
        if (prop->type == Cent_value_type_boolean) {
            suite->test->mute = prop->data.boolean;
        } else {
            Cent_ast* n = prop->n;
            Zinc_error_list_set(&data->errors, &n->loc, "expected boolean");
        }
...
{
    Apt_test* test = NULL;
    Apt_test_create(&test);
    Apt_test_list_add(&suite->list, test);
        ...
        if (item->kind == LAVA_DOM_TEXT) {
            ...
            if (Zinc_string_compare_str(&item->data.LAVA_DOM_BACKQUOTE.format, "cent")) {
                if (!seen_meta) {
                    ...
                    Cent_comp_unit_parse(ct->primary);
                    Cent_comp_unit_build(ct->primary);
                    Apt_parse_test_meta(data, test, ct->primary->value);
                    Cent_comp_table_destroy(ct);
                    free(ct);
                } else {
                    Zinc_string_add_string(&test->ast_path, path);
                    test->ast_bounds = item->data.LAVA_DOM_BACKQUOTE.bounds;
                }
            } else if (Zinc_string_compare_str(&item->data.LAVA_DOM_BACKQUOTE.format, "akela")) {
                Zinc_string_add_string(&test->source_path, path);
...
Apt_test* Apt__parse_test_case_tc = NULL;
void Apt_parse_test_meta(Apt_data* data, Apt_test* tc, Cent_value* value)
{
    if (value->type != Cent_value_type_dag) {
        Cent_ast* n = value->n;
...
{
    Apt_data* data = Apt__parse_test_suite_meta_prop_data;
    Apt_test* tc = Apt__parse_test_case_tc;

    if (Zinc_string_compare_str(name, "solo")) {
        if (prop->type == Cent_value_type_boolean) {
            tc->test->solo = prop->data.boolean;
        } else {
            Cent_ast* n = prop->n;
            Zinc_error_list_set(&data->errors, &n->loc, "expected boolean");
        }
        ...
        if (prop->type == Cent_value_type_boolean) {
            tc->test->mute = prop->data.boolean;
        } else {
            Cent_ast* n = prop->n;
            Zinc_error_list_set(&data->errors, &n->loc, "expected boolean");
        }
