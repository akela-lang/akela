...
{   
    ...
    /* line 1 */
    Worm_ast* level = Worm_ast_get(root, 0);
    Zinc_assert_ptr(level, "ptr level");
    Zinc_expect_int_equal(level->type, Worm_ast_type_level, "type level");

    Worm_ast* number = Worm_ast_get(level, 0);
    ...
    Worm_ast* scalar = Worm_ast_get(level, 1);
