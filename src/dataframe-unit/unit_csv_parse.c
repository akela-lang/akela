...
void DfUnit_CSVParseRowField(Zinc_test* test)
{
    ...
    Zinc_expect_int_equal(test, field_type, FieldTypeIntU, "types");

    Zinc_expect_int_equal(test, s->type, FieldTypeIntU, "type");

    Zinc_vector* raw0 = *(Zinc_vector**)ZINC_VECTOR_PTR(&s->raw, 0);
    Zinc_expect_true(test, Zinc_vector_match_str(raw0, "11"), "11 raw");

    Zinc_expect_u_long_equal(test, ZINC_VECTOR_U_LONG(&s->value, 0), 11, "11 value");
...
void DfUnit_CSVParseTypes(Zinc_test* test)
{
    ...
    Zinc_expect_int_equal(test, ZINC_VECTOR_INT(&s0->types, 0), FieldTypeBool, "s0 types 0");
    Zinc_expect_true(test, Zinc_vector_match_str(ZINC_VECTOR_VECTOR(&s0->raw, 0), "True"), "s0 raw 0");
    Zinc_expect_int_equal(test, ZINC_VECTOR_BOOL(&s0->value, 0), 1, "0 value 0");

    Zinc_expect_int_equal(test, ZINC_VECTOR_INT(&s0->types, 1), FieldTypeBool, "s0 types 1");
    Zinc_expect_true(test, Zinc_vector_match_str(ZINC_VECTOR_VECTOR(&s0->raw, 1), "False"), "s0 raw 1");
    ...
    Zinc_expect_int_equal(test, ZINC_VECTOR_INT(&s1->types, 0), FieldTypeFloat, "s1 types 0");
    Zinc_expect_true(test, Zinc_vector_match_str(ZINC_VECTOR_VECTOR(&s1->raw, 0), "1.2"), "s1 raw 0");
    Zinc_expect_double_equal(test, ZINC_VECTOR_DOUBLE(&s1->value, 0), 1.2, "s1 value 0");

    Zinc_expect_int_equal(test, ZINC_VECTOR_INT(&s1->types, 1), FieldTypeFloat, "s1 types 1");
    Zinc_expect_true(test, Zinc_vector_match_str(ZINC_VECTOR_VECTOR(&s1->raw, 1), "5.1"), "s1 raw 1");
    ...
    Zinc_expect_int_equal(test, ZINC_VECTOR_INT(&s2->types, 0), FieldTypeInt, "s2 types 0");
    Zinc_expect_true(test, Zinc_vector_match_str(ZINC_VECTOR_VECTOR(&s2->raw, 0), "-3"), "s2 raw 0");
    Zinc_expect_int_equal(test, ZINC_VECTOR_INT(&s2->value, 0), -3, "s2 value 0");

    Zinc_expect_int_equal(test, ZINC_VECTOR_INT(&s2->types, 1), FieldTypeIntU, "s2 types 1");
    Zinc_expect_true(test, Zinc_vector_match_str(ZINC_VECTOR_VECTOR(&s2->raw, 1), "80"), "s2 raw 1");
    ...
    Zinc_expect_int_equal(test, ZINC_VECTOR_INT(&s3->types, 0), FieldTypeIntU, "s3 types 0");
    Zinc_expect_true(test, Zinc_vector_match_str(ZINC_VECTOR_VECTOR(&s3->raw, 0), "50"), "s3 raw 0");
    Zinc_expect_int_equal(test, ZINC_VECTOR_INT(&s3->value, 0), 50, "s3 value 0");

    Zinc_expect_int_equal(test, ZINC_VECTOR_INT(&s3->types, 1), FieldTypeIntU, "s3 types 1");
    Zinc_expect_true(test, Zinc_vector_match_str(ZINC_VECTOR_VECTOR(&s3->raw, 1), "1"), "s3 raw 1");
    ...
    Zinc_expect_int_equal(test, ZINC_VECTOR_INT(&s4->types, 0), FieldTypeString, "s4 types 0");
    Zinc_expect_true(test, Zinc_vector_match_str(ZINC_VECTOR_VECTOR(&s4->raw, 0), "hello"), "s4 raw 0");
    Zinc_expect_vector_str(test, ZINC_VECTOR_VECTOR(&s4->value, 0), "hello", "s4 value 0");

    Zinc_expect_int_equal(test, ZINC_VECTOR_INT(&s4->types, 1), FieldTypeString, "s4 types 1");
    Zinc_expect_true(test, Zinc_vector_match_str(ZINC_VECTOR_VECTOR(&s4->raw, 1), "world"), "s4 raw 1");
    ...
    Zinc_expect_int_equal(test, ZINC_VECTOR_INT(&s5->types, 0), FieldTypeEmpty, "s5 types 0");
    Zinc_expect_true(test, Zinc_vector_match_str(ZINC_VECTOR_VECTOR(&s5->raw, 0), ""), "s5 raw 0");

    Zinc_expect_int_equal(test, ZINC_VECTOR_INT(&s5->types, 0), FieldTypeEmpty, "s5 types 0");
    Zinc_expect_true(test, Zinc_vector_match_str(ZINC_VECTOR_VECTOR(&s5->raw, 0), ""), "s5 raw 0");

    Zinc_expect_size_t_equal(test, s5->value.count, 0, "s5 value count");
