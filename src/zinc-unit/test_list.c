...
void test_list_1()
{
    ...
    *n2 = 2;
    ...
    Zinc_assert_ptr(x0, "ptr x0");
    Zinc_expect_ptr_equal(x0, n0, "x0");
    Zinc_expect_int_equal(*x0, 0, "0");

    int* x1 = Zinc_list_get(&l, 1);
    Zinc_assert_ptr(x1, "ptr x1");
    Zinc_expect_ptr_equal(x1, n1, "x1");
    Zinc_expect_int_equal(*x1, 1, "1");

    int* x2 = Zinc_list_get(&l, 2);
    Zinc_assert_ptr(x2, "ptr x2");
    Zinc_expect_ptr_equal(x2, n2, "x2");
    Zinc_expect_int_equal(*x2, 2, "2");
