...
void Zinc_unit_string_list_add(Zinc_test* test)
{
    ...
    Zinc_expect_null(test, bl.head, "head 0");
    Zinc_expect_null(test, bl.tail, "tail 0");

    Zinc_string_list_add(&bl, bn0);
    Zinc_expect_ptr_equal(test, bl.head, bn0, "head 1");
    Zinc_expect_ptr_equal(test, bl.tail, bn0, "tail 1");
    Zinc_expect_null(test, bn0->next, "next 1");
    Zinc_expect_null(test, bn0->prev, "next 1");

    Zinc_string_list_add(&bl, bn1);
    Zinc_expect_ptr_equal(test, bl.head, bn0, "head 2");
    Zinc_expect_ptr_equal(test, bl.tail, bn1, "tail 2");
    Zinc_expect_ptr_equal(test, bn0->next, bn1, "next bn0 2");
    Zinc_expect_ptr_equal(test, bn0->prev, NULL, "prev bn0 2");
    Zinc_expect_ptr_equal(test, bn1->next, NULL, "next bn1 2");
    Zinc_expect_ptr_equal(test, bn1->prev, bn0, "prev bn1 2");
...
void Zinc_unit_string_list_add_str(Zinc_test* test)
{
    ...
    Zinc_expect_string(test, &bn0->value, "one", "one");

    Zinc_string_node* bn1 = bn0->next;
    Zinc_expect_string(test, &bn1->value, "two", "two");
...
void Zinc_unit_string_list_count(Zinc_test* test)
{
    ...
    Zinc_string_list_add_str(&bl, "one");
    Zinc_string_list_add_str(&bl, "two");
    Zinc_expect_size_t_equal(test, Zinc_string_list_count(&bl), 2, "2");
    Zinc_string_list_destroy(&bl);
}
...
void Zinc_unit_string_list_split(Zinc_test* test)
{
    ...
    Zinc_string_node* bn0 = bl.head;
    if (!Zinc_expect_ptr(test, bn0, "ptr bn0")) {
        return Zinc_assert();
    }
    Zinc_expect_string(test, &bn0->value, "one", "one bn0");

    Zinc_string_node* bn1 = bn0->next;
    if (!Zinc_expect_ptr(test, bn1, "ptr bn1")) {
        return Zinc_assert();
    }
    Zinc_expect_string(test, &bn1->value, "two", "two bn1");

    Zinc_string_node* bn2 = bn1->next;
    if (!Zinc_expect_ptr(test, bn2, "ptr bn2")) {
        return Zinc_assert();
    }
    Zinc_expect_string(test, &bn2->value, "three", "three bn2");
...
void Zinc_unit_string_list_get(Zinc_test* test)
{
    ...
    if (!Zinc_expect_ptr(test, bf0, "ptr bf0")) {
        return Zinc_assert();
    }
    Zinc_expect_string(test, bf0, "one", "one bf0");

    Zinc_string* bf1 = Zinc_string_list_get(&bl, 1);
    if (!Zinc_expect_ptr(test, bf1, "ptr bf1")) {
        return Zinc_assert();
    }
    Zinc_expect_string(test, bf1, "two", "two bf1");

    Zinc_string* bf2 = Zinc_string_list_get(&bl, 2);
    if (!Zinc_expect_ptr(test, bf2, "ptr bf2")) {
        return Zinc_assert();
    }
    Zinc_expect_string(test, bf2, "three", "three bf2");
