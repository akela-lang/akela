        ...
        bool* has_id)
{
    ...
    Ake_AstCreate(&n);
    Ake_AstSet(n, AKE_AST_PROTOTYPE);
    ...
    if (dseq_node) {
        n->data.prototype.dseq = dseq_node;
        Ake_AstAdd2(n, dseq_node);
    }
        ...
        bool is_const)
{
    ...
    if (t0->type == Ake_token_id || type_only) {
        Ake_AstCreate(&n);
        Ake_AstSet(n, AKE_AST_DECLARATION);
...
Ake_Type* Ake_parse_type_function(Ake_parse_state* ps, Ake_Ast* n)
{
    ...
    if (proto->has_error) {
        ...
        Ake_Type_use_add_proto(ps, type, proto);
    }
...
Ake_Type* Ake_proto2type_use(Ake_parse_state* ps, Ake_Ast* proto) {
    Ake_Type *func = NULL;
    Ake_TypeCreate(&func);
    Ake_TypeSet(func, AKE_TYPE_FUNCTION);
    return Ake_Type_use_add_proto(ps, func, proto);
}
...
void Ake_Override_rhs(Ake_Type* type, Ake_Ast* rhs)
{
    ...
    if (rhs->kind == AKE_AST_ARRAY_LITERAL) {
        ...
        if (type->kind == AKE_TYPE_ARRAY) {
            ...
            Ake_Ast* p = rhs->data.array_literal.list.head;
            while (p) {
