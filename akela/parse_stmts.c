...
Cob_ast* parse_stmt(struct parse_state* ps);
Cob_ast* parse_while(struct parse_state* ps);
Cob_ast* parse_for(struct parse_state* ps);
void parse_for_range(struct parse_state* ps, Cob_ast* parent);
void parse_for_iteration(struct parse_state* ps, Cob_ast* parent);
Cob_ast* parse_module(struct parse_state* ps);
Cob_ast* parse_struct(struct parse_state* ps);
Cob_ast* parse_return(struct parse_state* ps);
Cob_ast* parse_let(struct parse_state* ps);
Cob_ast* parse_let_lseq(struct parse_state* ps);
Cob_ast* parse_let_rseq(struct parse_state* ps);
Cob_ast* parse_extern(struct parse_state* ps);
Cob_ast* parse_impl(struct parse_state* ps);

/* stmts -> stmt stmts' */
/* stmts' -> separator stmt stmts' | e */
/* NOLINTNEXTLINE(misc-no-recursion) */
Cob_ast* parse_stmts(struct parse_state* ps, bool suppress_env)
{
	Cob_ast* n = NULL;
	Cob_ast* last = NULL;
...
Cob_ast* parse_for(struct parse_state* ps)
{
	...
	if (c && c->type == Cob_ast_type_error) {
        n->type = Cob_ast_type_error;
    }
...
void parse_for_iteration(struct parse_state* ps, Cob_ast* parent)
{
	...
	Cob_ast* list = NULL;
    list = parse_expr(ps);
	if (list && list->type == Cob_ast_type_error) {
        parent->type = Cob_ast_type_error;
    }
    ...
    if (list) {
        Cob_ast_add(parent, list);
    }

	if (parent->type != Cob_ast_type_error) {
		Cob_ast* element = Ast_node_get(parent, 0);
		Cob_ast* element_type_node = Ast_node_get(element, 1);
	...
	if (n->type != Cob_ast_type_error) {
		if (a) {
			if (!a->tu) {
				...
				Cob_ast* fd = get_current_function(ps->st->top);
				if (!fd) {
...
Cob_ast* parse_impl(struct parse_state* ps)
{
    ...
    while (true) {
        ...
        if (t->type == token_fn) {
            Cob_ast* func = parse_function(ps, true, struct_type);
            Cob_ast_add(n, func);
