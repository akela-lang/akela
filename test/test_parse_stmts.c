...
void test_parse_function()
{
    ...
    parse_setup("function foo() let x::Int64; x+1; 5+4 end", &cu);
	assert_no_errors(&cu.el);
	assert_true(cu.valid, "parse_setup valid");
...
void test_parse_function2()
{
	...
	struct ast_node* f_stmts = ast_node_get(f_node, 1);
	assert_ptr(f_stmts, "ptr b");
	expect_int_equal(f_stmts->type, ast_type_stmts, "type f_stmts");

	struct ast_node* d = ast_node_get(f_stmts, 0);
	assert_ptr(d, "ptr d");
	...
	struct ast_node* g = ast_node_get(f_stmts, 1);
	assert_ptr(g, "ptr g");
...
void test_parse_function3()
{
	...
	struct ast_node* a = ast_node_get(proto, 0);
	assert_ptr(a, "ptr a");
	expect_int_equal(a->type, ast_type_id, "id");

	struct ast_node* seq = ast_node_get(proto, 1);
	...
	struct ast_node* b = ast_node_get(f, 1);
	assert_ptr(b, "ptr b");
...
void test_parse_function5()
{
    ...
    parse_setup("function foo(x::Int64) let x::Int64 end", &cu);
...
void test_parse_dot_error_left_non_module()
{
    ...
    parse_setup("function foo() end; module math let pi::Float64 = 3.14 end; true.1", &cu);
	expect_has_errors(&cu.el);
	expect_false(cu.valid, "valid");
...
void test_parse_dot_error_right_not_identifier()
{
    ...
    parse_setup("function foo() end; module math let pi::Float64 = 3.14 end; math.\"hello\"", &cu);
	expect_has_errors(&cu.el);
	expect_false(cu.valid, "valid");
...
void test_parse_struct()
{
    ...
    expect_int_equal(var_rseq->type, ast_type_let_rseq, "var_rseq var_rseq");
...
void test_parse_var()
{
    ...
    assert_ptr(var, "ptr var");
    expect_int_equal(var->type, ast_type_let, "var");
